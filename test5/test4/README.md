...
## 实验步骤
### 步骤一 用户创建和空间分配
创建新的表空间和用户laiyu，并分配给laiyu表空间的查询权限以及创建视图的权限
#### 1） 创建表空间
使用system用户创建表空间USERS02，并给表空间分配数据文件，数据文件大小均为100，表空间大小为200M.
```sql
...
```sql
CREATE USER laiyu IDENTIFIED BY 123
...
```
* 分配角色CONNEC、RESOURCE，以便用户可以连接数据库，创建资源。
```sql
ALTER USER laiyu ACCOUNT UNLOCK;
GRANT "CONNECT" TO laiyu WITH ADMIN OPTION;
GRANT "RESOURCE" TO laiyu WITH ADMIN OPTION;
```
*分配系统权限"CREATE VIEW"，以便创建视图
```sql
ALTER USER laiyu DEFAULT ROLE "CONNECT","RESOURCE";
GRANT CREATE VIEW TO laiyu WITH ADMIN OPTION;
```
![](grantly.jpg)
### 步骤二 删除已经存在的表和序列
删除表和序列
...
![](COrders.png)
##### ② 创建本地分区索引
在laiyu的ORDERS表中创建本地分区索引ORDERS_INDEX_DATE
```sql
...
在laiyu的ORDERS表中创建本地分区索引ORDERS_INDEX_CUSTOMER_NAME
```sql
...
![](Cindex1.png)
##### ③ 设置唯一索引ODERS_PK
在laiyu用户的ORDERS表中创建全局分区索引ORDERS_PK
```sql
...
* 7.查询部门表，统计每个部门的销售总金额。
```sql
SELECT department_name,sum( Product_Num*Product_Price - Discount) as total_money
from departments,employees,orders,order_details 
where departments.department_id=employees.department_id and
employees.employee_id=orders.employee_id
and orders.order_id=order_details.order_id group by department_name;
```
![](s7.png)
