...
### 步骤一 用户创建与空间分配
#### 1. 创建表空间
登录用户system_pdborcl,创建新的表空间USERS02，并给表空间分配数据文件，数据文件大小均为100，表空间大小为200M.
```sql
Create Tablespace Users02
datafile
'/home/oracle/app/oracle/oradata/orcl/pdborcl/pdbtest_users02_1.dbf' SIZE 100M AUTOEXTEND ON NEXT 256M MAXSIZE UNLIMITED,
'/home/oracle/app/oracle/oradata/orcl/pdborcl/pdbtest_users02_2.dbf' SIZE 100M AUTOEXTEND ON NEXT 256M MAXSIZE UNLIMITED
...
```sql
CREATE USER ycl IDENTIFIED BY 123
...
* 为用户ycl分配以下权限:
    ...
    * 创建触发器权限
```sql
ALTER USER ycl QUOTA UNLIMITED ON USERS;
ALTER USER ycl QUOTA UNLIMITED ON USERS02;
ALTER USER ycl ACCOUNT UNLOCK;
GRANT "CONNECT" TO ycl WITH ADMIN OPTION;
GRANT "RESOURCE" TO ycl WITH ADMIN OPTION;
ALTER USER ycl DEFAULT ROLE "CONNECT","RESOURCE";
GRANT CREATE VIEW TO ycl WITH ADMIN OPTION;
GRANT dba to ycl;
```
![](images\1-3权限分配.jpg)
...
ENABLE;
```
![](images\3-3-2添加约束.jpg)

#### 4.建临时表: ORDER_ID_TEMP
临时表存储在临时表空间中，而不是存储在USERS和USERS02表空间中。
```sql
...
```
![](images\3-4创建临时表ORDER_ID_TEMP.jpg)
...
```
![](images\3-5-1创建表ORDERS.jpg)

- 创建本地分区索引
在ycl的ORDERS表中创建本地分区索引ORDERS_INDEX_DATE
```sql
...
- 在ycl的ORDERS表中创建本地分区索引ORDERS_INDEX_CUSTOMER_NAME
```sql
...
NOPARALLEL;
```
![](images\3-5-2创建本地分区索引.jpg)

- 设置唯一索引ODERS_PK
在ycl用户的ORDERS表中创建全局分区索引ORDERS_PK
```sql
...
NOPARALLEL;
```
![](images\3-5-3设置唯一索引ODERS_PK.jpg)
...
ENABLE;
```
![](images\3-5-4设置主键和外键.jpg)

#### 6.创建表ORDER_DETAILS
ORDER_DETAILS是ORDERS的从表
```sql
...
;
```
![](images\3-6-1创建表ORDER_DETAILS.jpg)

- 设置唯一键、主键、索引、约束
因为ORDERS_DETAILS表中没有订单时间，所以不能像ORDERS一样按时间分区存储，所以利用引用分区的功能，从表按主表的分区方案与主表存储在同一个分区。
```sql
--设置唯一键
...
ENABLE;
```
![](images\3-6-2设置唯一键、主键、索引、约束.jpg)

### 步骤4

因为当修改订单表的折扣或订单详细表做了增加、修改和删除操作后，都需要重新计算应收货款Table_Receivable，所以设计三个触发器实现功能。
...
```
![](images\4-3触发器3.jpg)
![](images\4-3-2触发器3.jpg)

### 步骤五 创建序列和视图
#### 1.创建序列
因为插入订单数据时需要用到自增的ID，所以需要设计两个序列。分别用于给ORDERS和ORDERS_DETAILS表的ID赋值
```sql
...
```
![](images\10-1查询某个员工的信息.jpg)

- 查询2：递归查询某个员工及其所有下属，子下属员工
```sql
...
```
![](images\10-2递归查询某个员工及其所有下属，子下属员工.jpg)

- 查询3：查询订单表，并且包括订单的订单应收货款: Trade_Receivable= sum(订单详单表.ProductNum*订单详单表.ProductPrice)- Discount。
```sql
select order_details.order_id, Product_Num*Product_Price - Discount as Trade_Receivable from order_details,orders;
``` 
![](images\10-3查询订单表.jpg)

- 4.查询订单详表，要求显示订单的客户名称和客户电话，产品类型用汉字描述。
```sql
select customer_name,customer_tel,product_type 
from orders,order_details,products 
where order_details.order_id=orders.order_id 
and order_details.product_name=products.product_name;
```
![](images\10-4查询订单详表.jpg)

- 5.查询出所有空订单，即没有订单详单的订单。
```sql
select order_id from orders WHERE order_id not in(select order_id from order_details);
```
![](images\10-5查询所有空订单.jpg)

- 6.查询部门表，同时显示部门的负责人姓名。
```sql
select departments.department_id,department_name,name 
from departments,employees 
where departments.department_id=employees.department_id;
```
![](images\10-6查询部门表.jpg)

- 7.查询部门表，统计每个部门的销售总金额。
```sql
SELECT department_name,sum( Product_Num*Product_Price - Discount) as total_money from departments,employees,orders,order_details where departments.department_id=employees.department_id and
employees.employee_id=orders.employee_id and orders.order_id=order_details.order_id group by department_name;
```
![](images\10-7查询部门表，统计每个部门的销售总金额.jpg)

## 实验总结：
通过本次实验,我们学习了Oracle表和视图的概念，如何使用SQL语句创建表，学习Select语句进行数据的增删查改以及触发器的使用等等。比较应该留意的是最开始的授权,如果这一步没有做好,后面在创建视图或者表,分区索引,设置触发器时都会引发错误。
